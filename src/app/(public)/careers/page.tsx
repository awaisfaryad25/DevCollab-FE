          ...
          <ul className="grid gap-3 sm:grid-cols-2">
            {perks.map((perk) => (
              <li key={perk} className="flex items-center gap-3 text-sm text-foreground">
                <span className="flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-violet-100 text-violet-600 text-xs">✓</span>
                {perk}
              </li>
            ))}
          </ul>
          ...
          <div className="space-y-4">
            {roles.map((role) => (
              ...
              >
                ...
                <ArrowRight className="h-4 w-4 text-muted-foreground transition-transform group-hover:translate-x-1 group-hover:text-violet-600" />
              </Link>
            ))}
          </div>
