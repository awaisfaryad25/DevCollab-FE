...
import {
  Users, CreditCard, FolderKanban, TrendingUp, TrendingDown, ArrowRight, UserPlus, ShieldAlert,
...
const activity = [
  {
    ...
    text: "Ali Raza signed up via Google OAuth",
    time: "2m ago",
  },
  {
    icon: CreditCard,
    color: "text-emerald-600 bg-emerald-50 dark:bg-emerald-950",
    text: "Sara Ahmad upgraded to Pro — $19/mo",
    time: "14m ago",
  },
  {
    icon: ShieldAlert,
    color: "text-red-500 bg-red-50 dark:bg-red-950",
    text: "Failed login × 3 — user@example.com",
    time: "1h ago",
  },
  {
    icon: Building2,
    color: "text-blue-600 bg-blue-50 dark:bg-blue-950",
    text: "New workspace 'TechStart PK' created",
    time: "3h ago",
  },
  {
    icon: CheckSquare,
    color: "text-teal-600 bg-teal-50 dark:bg-teal-950",
    text: "Project 'DevCollab App' marked complete",
    time: "5h ago",
  },
  {
    icon: UserPlus,
    color: "text-violet-600 bg-violet-50 dark:bg-violet-950",
    text: "Maria Khan signed up via email",
    time: "6h ago",
  },
];
...
const maxBar = Math.max(...weeklySignups.map((d) => d.value));
    ...
    <>
      <div className="space-y-6 p-4 lg:p-6">
        ...
        <div>
          ...
          <p className="mt-0.5 text-sm">Welcome back — here's what's happening today.</p>
        </div>
              ...
              >
                <div className="flex items-start justify-between">
                  <div>
                    ...
                    <p className="mt-1.5 text-2xl font-semibold">{s.value}</p>
                  </div>
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
          {quickStats.map((q) => (
            ...
            >
              <p className="text-lg font-semibold">{q.value}</p>
              <p className="mt-0.5 text-[11px]">{q.label}</p>
            </div>
          ))}
        </div>
        ...
        <div className="grid gap-6 lg:grid-cols-2">
          ...
          <div className="rounded-xl border border-border bg-background p-5">
            ...
            <p className="mb-4 text-xs">329 total signups this week</p>
            <div className="flex items-end gap-2 h-60">
              {weeklySignups.map(({ day, value }) => (
                <div
                  key={day}
                ...
                >
                  ...
                  <div
                    className="w-full rounded-t-md bg-secondary hover:bg-primary transition-colors cursor-pointer"
                    style={{ height: `${(value / maxBar) * 120}px` }}
                    title={`${day}: ${value} signups`}
                  />
            ...
            <div className="mb-4 flex items-center justify-between">
              <div>
                ...
                <p className="mt-0.5 text-xs text-muted-foreground">
                  Latest signups across all plans
                </p>
              </div>
              <Link
                href="/users"
        ...
        <div className="rounded-xl border border-border bg-background p-5">
          <div className="mb-4 flex items-center justify-between">
            <div>
              ...
              <p className="mt-0.5 text-xs text-muted-foreground">
                Platform-wide events in real time
              </p>
            </div>
            <Link
              href="/activity"
